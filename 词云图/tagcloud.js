	...
	TagCloud.prototype = {
		...
		update: function() {
			...
			for (var j = 0, len = self.items.length; j < len; j++) {
				...
				self.items[j].element.style.left = self.items[j].x + (self.box.offsetWidth - self.items[j].offsetWidth) / 2 +
					"px";
				self.items[j].element.style.top = self.items[j].y + (self.box.offsetHeight - self.items[j].offsetHeight) / 2 +
					"px";
